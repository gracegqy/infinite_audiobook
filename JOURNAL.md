...
is retired) and that the curation-mode selector renders. Then, asked to
authorize a paid batch, she pushed back on the number: under $0.50, "as low as
possible with engineering."

### The estimate I had given her was wrong, and finding out was the whole lever

I had quoted ~$0.23 — run 4's cost, a batch of **8** with a starved 6-search
budget. The actual `--build-pool` command uses `POOL_BATCH_SIZE = 40`, and Entry
29's scaling gives it 120 searches: **$1.20 of search fees before a single
token**, ~$2.40 all in. Caught before spending, by reading the code instead of
trusting my own prior sentence.

Pricing the batch properly exposed the real structure: search was ~48% of the
bill directly and drove most of the token cost too. And searches bought exactly
two things — a correct `source_ref` and a reputation claim. **Both are available
for free**, which makes the search redundant rather than expensive:

- **Classics:** the Gutenberg catalog, already built in Entry 29.
- **Modern:** the creepypasta wiki's MediaWiki API — which `fetch.py` was
  already calling — exposes two editorially-curated categories, `PotM` (Pasta of
  the Month, 135 pages) and `Spotlighted Pastas` (74), zero overlap. All 209
  titles plus their byte sizes come back in **5 free API calls**; 200 fall
  inside the length bounds.

The page title IS the argument `fetch_creepypasta` takes, so the Entry-27
failure mode — 6 stub/deleted pages and 2 `missingtitle` refs out of 15 — is not
mitigated here, it is structurally impossible.

### Why this is a registry and not "a mode that also reads creepypasta"

Grace's ruling, and it was the right one: the modes have to work for channels
that do not exist yet. The Gutenberg catalog generalises (it matches LoC
subjects and bookshelves, so sci-fi or mystery work); **the creepypasta wiki is
horror by nature**, and wiring it into a mode would have hardcoded horror into
the pipeline against AMENDMENT_01 — the first sci-fi channel would have received
horror, or silently nothing.

So `pipeline/sources.py` holds sources that DECLARE coverage, and the registry
asks. Verified through the API on three channel shapes:

| channel | gutenberg-catalog | creepypasta-wiki |
|---|---|---|
| horror / en | ✓ | ✓ |
| science fiction / en | ✓ | — "horror-only source; channel genre/topics do not ask for horror" |
| science fiction / zh | ✓ | — "English-language wiki; channel language is zh" |

A channel no source covers raises `NoFreeSource` naming every reason and the way
forward. It never returns an empty pool and never falls back to the paid path —
a silent switch to spending is what AMENDMENT_04 A forbids. This also pays off
the "source-class registry" debt standing since Entry 16.

### Three modes, and what the run actually cost

`free` ($0, no model call) · `free_llm` (model picks from the free shortlist,
no web search) · `llm` (~$0.75, the only mode that reaches beyond the registry).
`catalog` is aliased to `free` rather than migrated — the stored setting keeps
working and still means the same thing.

First real `free_llm` run: **$0.0154**, 4,940 input / 553 output tokens, zero
searches. Sonnet 5 picked Poe, five Lovecrafts, Chambers, "The Night Wire" — the
canon, off a list the pipeline built for nothing. **43× cheaper than the paid
batch it replaces**, and 7× under Grace's ceiling.

### The first run failed in two ways, and both fixes are structural

1. **Balance regressed to 9 gutenberg / 3 creepypasta** off a 36/36 shortlist.
   The soft BALANCE clause lost — the same defeat Entries 27-28 took twice, from
   the same cause. The correction is to stop asking: `apply_class_quotas` is a
   pure function that round-robins the model's ranked picks across source
   classes. The model ranks (which it is good at); code holds the ratio (which
   it is not). Re-run: **6/6, enforced.**
2. **Two picks were Gutenberg collection volumes** (Dracula's Guest 306k chars,
   The King in Yellow 398k). The free verifier caught them exactly as designed,
   but they burned 2 of 12 slots. Fix: the call now ranks `batch + 6` and
   `build_pool` backfills from the spares after verification — a spare costs ~20
   output tokens where a re-run costs a whole call. Re-run: 4 collections
   rejected, all replaced, **12/12 usable** at **$0.0176**.

### Phase 5 gate criterion 3 — CLOSED

The channel-edit curation diff, owed since Entry 26 and previously priced at a
~$1.50 pair of batches, cost **$0.0264**. Setting
`exclusions=[Lovecraft, cosmic horror]`, `topics=[Victorian ghost stories]`:

- **dropped:** The Dunwich Horror, The Colour Out of Space — precisely the two
  Lovecraft cosmic-horror titles
- **added:** The Golgotha Dancers, Beyond the Middle of Nowhere
- 6 unchanged, all of which the edit gave no reason to move

All three Phase 5 gate criteria now pass. Only `/code-review` is owed.

### Honest limits

- **Canon coverage is thin.** Of nine famous creepypastas I checked, only NoEnd
  House and Ted the Caver are in the editorial categories. It is *editorial*
  reputation, not *popular* canon. This lands softly only because the library
  already consumed the canon — the editorial picks are the fresh supply.
- **Reach is bounded.** r/nosleep and anything outside these two sources still
  needs `llm` mode. That is what it is now for.
- **Supply is finite:** 514 classics + 200 modern ≈ 240 worker cycles. Long, not
  infinite, and it cannot grow on its own.
- **It retires the Entry-28 question rather than answering it.** The rebalanced
  paid prompt is still unverified end-to-end and now probably always will be.
- **`DEFAULT_CURATION_MODE` is still `llm`** — no behavior changes without
  Grace. The footgun that leaves: `--build-pool` without touching Settings still
  costs ~$2.40. One dropdown change fixes it; I did not make it for her.

### Grace's own click, found at close

The live DB turned out to hold `curation_mode = catalog`, written 03:54:35 —
Grace trying the selector while this work was in flight, not a stray write from
a sandbox (checked before concluding anything, per the Entry-26 rule). The alias
resolved it to `free`, which is exactly what the alias is for. Flagged in STATE
because a value set while testing a dropdown is not necessarily the mode she
wants to keep.

### Verification

189 tests green (was 151; +38). The live library and its DB were **not touched**
— every run above went through `HR_DATA_DIR` against a WAL-consistent snapshot,
per the Entry-26 rule. `catalog.build_pool` was deleted rather than left beside
`freepool.build_pool` as a second build path; the catalog's pure logic stays and
is driven by its adapter. The R11 ledger write was centralized into
`db.record_curation_run` (it was about to become a third copy).

**Spend not in the live ledger:** these runs cost **$0.0594** of real API calls
($0.0154 + $0.0176 + $0.0264), but their ledger rows live in sandbox databases,
so `curation_runs` on the live DB does not show them. Recorded here instead of
being quietly dropped, and carried into STATE's spend section.

Measurements invalidated by this change: my own "~$0.23 per batch" quote from
earlier this session (it described a batch of 8, not the 40 the command runs —
the real figure was ~$2.40). No previously journaled measurement changes: Entry
28's cached-batch numbers were correct for the batch size they described.

## Entry 31 — 2026-07-28 — JOURNAL reordered newest-first

Grace asked for the journal to read newest-first so the current state of the
project is the first thing visible instead of the last. All 31 entries were
reversed by script and verified word-for-word identical to the previous file
(same heading set, exact reverse order, identical whitespace-normalized token
list) — no entry text was edited. The ordering convention is now stated in the
header above and in CLAUDE.md's session-close ritual so future sessions prepend.

Note for readers: the duplicate heading "Entry 13 — Correction: bank paths
updated after meta-folder rename" is a pre-existing misnumbering (it should have
been 14b or 15). Reversal preserved its file position relative to its
neighbours; it now sits between Entry 15 and Entry 14. Left as-is — renumbering
it would be an edit to an existing entry.

Measurements invalidated by this change: none (presentation only).
